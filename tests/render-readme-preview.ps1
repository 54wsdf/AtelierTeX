...
$previews = @(
    [pscustomobject]@{
        Name = 'feature'
        PdfRelative = "build/$Engine/full-specimen/main.pdf"
        PreviewRelative = 'docs/assets/rendered/ateliertex-feature-page1'
    },
    [pscustomobject]@{
        Name = 'symposium'
        PdfRelative = "build/$Engine/full-specimen-symposium/main.pdf"
        PreviewRelative = 'docs/assets/rendered/ateliertex-symposium-page1'
    },
    [pscustomobject]@{
        Name = 'essay'
        PdfRelative = "build/$Engine/essay-longread-demo/main.pdf"
        PreviewRelative = 'docs/assets/rendered/ateliertex-essay-page1'
    }
)

# 防退化说明：README 预览必须由当前完整样张编译生成，以保证公共框架、书目
# 数据与页面预览保持一致。
& pwsh (Join-Path $PSScriptRoot 'compile-full-specimen.ps1') -Engine $Engine
if ($LASTEXITCODE -ne 0) { throw "AtelierTeX full specimen build failed: $Engine" }
...
# 防退化说明：文件存在或 PNG 签名不足以证明图像完整；这里逐张解码末端像素，
# 并检查 A4 尺寸与不透明 RGB 色彩类型。
